...
angular.module('myApp.news', ['ngRoute'])
    ...
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/mobile/news/:id', {
            templateUrl: 'mobile/templates/user/news.html',
            controller: 'NewsCtrl'
        });
    }])
...
//      }
        NewsService.news($routeParams.id).then(function () {
            ...
            $scope.News.content= $scope.News.content.replace(/(font-size:[^><;"]*(;)?)/ig,"");
            $scope.News.content= $scope.News.content.replace(/(line-height:[^><;"]*(;)?)/ig,"");
