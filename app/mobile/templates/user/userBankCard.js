...
angular.module('myApp.userBankCard', ['ngRoute'])
	...
	.controller('UserBankCardCtrl', function($scope, $mdDialog, UserService, UserBankCardService, $http, $location) {
...
//	$http.get("http://yunkus.com/demo/angularjs/angularjs-city-picker/json/city-picker.json").success(function(data) {
...
//				$scope.bankIndex = 2;
			...
			$scope.bankInfo();
			//          UserBankCardService.synUserCard(userId, token).success(function () {
			//              $scope.userCard = (UserBankCardService.getUserCards())[0];
			//              var banks = $scope.banks;
			//              for (var i = 0; i < banks.length; i++) {
			//                  if (banks[i]['id'] == $scope.userCard['type']) {
			//                      $scope.userCard['type'] = banks[i];
			//                      break;
			//                  }
			//              }
			//          });
...
//			console.log($scope.userCard);
			...
			//          for (var i = 0; i < banks.length; i++) {
			//              if (banks[i]['label'] == cardData['type']['label']) {
			//                  cardData.type = banks[i]['id'];
			//                  break;
			//              }
			//          }
